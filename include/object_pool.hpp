...
// "Isn't  object_pool.hpp  including  pool_handle.hpp  circular?"

// No.  pool_handle.hpp  includes only  <cassert> ,  <utility>  and  <cstddef>  — nothing of yours. It never names  ObjectPool ; it takes  Pool  as a template parameter. That genericity is precisely what makes the one-way include legal
/*  ---What we want
...
// template<typename T, typename Pool>  -> no need for this , #include "pool_handle.hpp" (legal to redeclare but redundant (not a second definiton)- becomes an error if the declarations conflict eg- template<typename T> class PoolHandle; erro:wrong parameter count)
// class PoolHandle;
...
public:
    ...
    //Similar to std::make_unique for unique pointer, we will do objectPool::make() for Pool Handler
    //make()- RAII front door: acquire + wrap in one expression
    //Why it exists: Making handler in 2 steps may leave a hole
    // Particle* p =pool.acquire(1.f,2.f);// slot is ours now
    // PoolHandle<Particle,Pool>h{p,&pool};//-> once acquire() succeeds, p represents an owned resource, but no RAII object protects it yet. Any added return,throw or throwing function before wrapping it leaks the slot 
    //auto h=pool.make(...) removes that vulnerable intermediate state by acquiring and wrapping in one expression.
    //make() removes the hole- acquire()'s result feeds straight into a noexcept constructor (of pool handle) inside a single full expression


    //Return type of make() function is PoolHandle<T,ObjectPool> (obvious)
    //ObjectPool instead of ObjectPool<T,Capacity,ExhaustionPolicy> bcz inside a class template the bare name 'ObjectPool' is the injected-class-name, meaning ObjectPool<T,Capacity,ExhaustionPolicy>- the exact current instantiation

    //noexcept condition is similar to acquire()'s, bcz PoolHandle's constructor is itself noexcept
    //the only things that can throw here are T's constructor and exhaustion policuy7

    template<typename... Args>
    [[nodiscard]] PoolHandle<T,ObjectPool> make(Args&&... args) noexcept(std::is_nothrow_constructible_v<T,Args...> && noexcept(ExhaustionPolicy::on_exhausted())){ //nodiscard bcz handle calls acquire() and a slot is occupied, so thats not good- bug at call site
        //this= ObjectPool* (which is exactly Pool* from Handlers perspective )
        //obviously our return type can't be PoolHandle<T,ObjectPool>& (i.e we cant return by reference bcz we are creating the PoolHandle inside the functions stack {had we allocated the handle on heap it would have been different thing altogether})
        //remember - there is no copy constructor/assignment for pool handler, only move constructor/assignment 
        return PoolHandle<T,ObjectPool>{acquire(std::forward<Args>(args)...),this};//this is a pure temporary (prvalue) since the handle has no name - Post C++17 it uses guranteed copy elision [BIG BENEFIT OF C++17], pre c++17 it would have called the move constructor of PoolHandle to move the PoolHandle object from function stack to the caller's memory
        //directly doing return {acquire(std::forward<Args>(args)...),this};  would give a compile error bcz this is a copy list initialization of the object PoolHandle and the constructor of PoolHandle is Explicit (to prevent accidental ownership transfers)
        //therefore we had to do a direct list initialization - PoolHandle<T,ObjectPool>{acquire(std::forward<Args>(args)...),this}
        //NullOnExhaustion acquire() returns a nullptr, and we hand back an empty handle {nullptr,this}
        //this still satisfies the invariant - ptr_!=nullptr => pool_!=nullptr & ~PoolHandle() only calls release when ptr_!=nullptr , so nothing disastorous happens
    }

    //Does make() leak if T's constructor throws ? Ans- No . acquire() restores the free-list link and rethrows before the handle is ever constructred . Nothing was commited and free_head_ and in_use_ are untouched
